...
const { initDatabase, getDb, getTursoClient, isUsingTurso, saveDatabase } = require('./database');
...
// Mynd upload - base64
app.post('/api/verkefni/:id/mynd', express.json({ limit: '10mb' }), async (req, res) => {
  try {
    const { mynd } = req.body;
    if (!mynd) return res.status(400).json({ error: 'Enga mynd' });

    await dbRun('UPDATE verkefni SET mynd = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [mynd, req.params.id]);
...
app.get('/api/verkefni/:id/timi', async (req, res) => {
  try {
    const timi = await dbAll('SELECT * FROM timaskraning WHERE verkefni_id = ? ORDER BY dagsetning DESC', [req.params.id]);
    res.json(timi);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/verkefni/:id/adkeypt', async (req, res) => {
  try {
    const adkeypt = await dbAll('SELECT * FROM adkeypt WHERE verkefni_id = ? ORDER BY created_at DESC', [req.params.id]);
    res.json(adkeypt);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/verkefni/:id/pdf', async (req, res) => {
  try {
    ...
    const timi = await dbAll('SELECT * FROM timaskraning WHERE verkefni_id = ? ORDER BY dagsetning', [req.params.id]);
    const adkeypt = await dbAll('SELECT * FROM adkeypt WHERE verkefni_id = ? ORDER BY created_at', [req.params.id]);
...
app.get('/api/google-doc/:docId', async (req, res) => {
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: 'Villa við að sækja skjal - athugaðu að það sé opið fyrir "Anyone with the link"' });
    }
